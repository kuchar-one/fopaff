...
def operator_o1(N, a):
    """
    Return the operator O_1(N, a) = (position(N)^2 - a^2 * qeye(N))^2.
    ...
    -------
    Qobj
        The operator O_1(N, a).
    """
    return (qt.position(N)**2 - a**2 * qt.qeye(N))**2
...
def high_dim_generator(u, phi, c, k):
    ...
    high_dim = ((qt.position(dim)**2 - u**2 * qt.qeye(dim))**2 + c*u*gamma(k+1)/(np.sqrt(np.pi)*gamma(k+0.5))*(-1/4)**k * (minus.expm() - plus.expm())**(2*k))
    
    print(f"Generating pre-truncation form of the SQE Operator for u = {u}, phi = {phi}, c = {c}, k = {k}...")
    
    os.makedirs('cache/operators', exist_ok=True)
    qt.qsave(high_dim, f'cache/operators/high_dim_u{u:.2f}_phi{phi:.2f}_c{c:.2f}_k{k:.2f}')
...
def p_projector(N):
    """
    Generate the projector onto the even-parity subspace of a mode.

    Parameters
    ----------
    N : int
        The dimension of the mode.

    Returns
    -------
    Qobj
        The projector onto the even-parity subspace of the mode.
    """
    result = 0
    for n in range(N):
        coeff = (-1j)**n / np.sqrt(2**n * factorial(n))
        hermite_val = hermite(n)(0)
        result += coeff * hermite_val * qt.basis(N, n)
    return result
    
def operator_o2odd(N, a, max):
    plus = 1j * (qt.momentum(N) * a + np.pi/2)
    minus = -1j * (qt.momentum(N) * a + np.pi/2)
    return (-1/4)**max * (minus.expm() - plus.expm())**(2*max)

def operator_o2(N, a, max):
    plus = 1j * qt.momentum(N) * a
    minus = -1j * qt.momentum(N) * a
    return (-1/4)**max * (minus.expm() - plus.expm())**(2*max)

def gaussian_limit(u, c):
    return min([u*c/np.pi, minimize_scalar(lambda r: float(u**4+3/(4*exp(4*r)) - u**2/exp(2*r) + u*c/pi*jtheta(3, -1/2*pi, exp(-(u**2*exp(2*r))))), bounds=[-10, 10], method="bounded").fun])

def normal_factor(u, k):
    return float(u * 4**k * (fac(k))**2 / (pi * fac(2 * k)))
...
def beam_splitter(N, one_in, two_in, theta = np.pi/4):
    ...
    unitary = (-theta*(destroy_one.dag()*destroy_two-destroy_two.dag()*destroy_one)).expm()
    if one_in.type == 'ket':
        one_in = qt.ket2dm(one_in)
    if two_in.type == 'ket':
        two_in = qt.ket2dm(two_in)
    input_dm = qt.tensor(one_in, two_in)
    
    return unitary.dag()*input_dm*unitary
    
def measure_mode(N, two_mode_dm, projector, mode):
    
    """projection measurement of two mode state
    ...
    if mode == 1:
        measurement = qt.tensor(projector, qt.identity(N))
        return ((two_mode_dm*measurement).ptrace(1)).unit()
    
    elif mode == 2:
        measurement = qt.tensor(qt.identity(N), projector)
        return ((two_mode_dm*measurement).ptrace(0)).unit()
...
def beam_splitter(N, one_in, two_in, theta=np.pi/4):
    ...
    generator = -theta * (destroy_one.dag() * destroy_two - destroy_two.dag() * destroy_one)
...
def measure_mode(N, two_mode_dm, projector, mode):
    ...
    measurement = qt.tensor(projector, id_N) if mode == 1 else qt.tensor(id_N, projector)
...
def high_dim_marek_generator():
    
    """
    Generate and save a high-dimensional operator for the MAREK protocol.
    ...
    -------
    oper
        The high-dimensional MAREK operator.
    """
    dim = 5000
    ...
    qt.qsave(high_dim, f'operators/high_dim_marek')
    
    return high_dim

def marek_operator0_new(N):
    ...
    Qobj
        The truncated high-dimensional operator.
    """
    filename = f'cache/operators/high_dim_marek'
    
    if os.path.isfile(f'{filename}.qu'):
        high_dim = qt.qload(filename)    
    else:
        high_dim = high_dim_marek_generator()
...
def catfid(N, state, a, c, k, projector):
    ...
    ideal_state = ((qt.displace(N, a/2) + qt.displace(N, -a/2))*qt.squeeze(N,np.log(2)/2)*qt.basis(N,0)).unit()
    output_state = measure_mode(N, beam_splitter(N, state, qt.basis(N,0)), projector, 1)
